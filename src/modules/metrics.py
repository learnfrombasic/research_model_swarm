...
def ensemble_based_on_utility(preds, utility_list, top_k):
    ...
    for i in range(len(preds[0])):
        ...
        max_frequency_achieved_flag = [
            pred_frequency_dict[pred] == max_frequency for pred in pred_frequency_dict
        ]
...
def overall_metrics(name, eval_type, top_k=10):
    ...
    search_path = Path("search") / name
    particle_paths = [p.name for p in search_path.iterdir() if p.is_dir()]

    if (
        eval_type == "multiple_choice"
        or eval_type == "AbstainQA"
        or eval_type == "multitask"
    ):
        golds = None
        starting_preds = []
        starting_utility = []
        ...
        for i in range(len(particle_paths)):
            if "particle_" + str(i) in particle_paths:
                ...
                preds_file = search_path / particle_path / "personal_best" / "preds.json"
                with open(preds_file, "r") as f:
                    particle_data = json.load(f)
                    ending_preds.append(particle_data)
                golds_file = search_path / particle_path / "personal_best" / "golds.json"
                with open(golds_file, "r") as f:
        ...
        utility_file = search_path / "utility_scratchpad.json"
        ...
        try:
            for i in range(len(particle_paths)):
                if "particle_" + str(i) in particle_paths:
                    particle_path = "particle_" + str(i)
                    preds_file = search_path / particle_path / "now" / "preds.json"
                    with open(preds_file, "r") as f:
                        particle_data = json.load(f)
                        starting_preds.append(particle_data)
                    golds_file = search_path / particle_path / "now" / "golds.json"
                    with open(golds_file, "r") as f:
                        gold_data = json.load(f)
                        if golds and not eval_type == "AbstainQA":
                            assert golds == gold_data
            ...
            with open(utility_file, "r") as f:
                ...
                for i in range(len(particle_paths)):
                    if "particle_" + str(i) in particle_paths:
                        particle_path = "particle_" + str(i)
                        starting_utility.append(
                            utility_data[particle_path + "_history"][0]
                        )
        except:
        ...
        assert (
            len(starting_preds)
            == len(starting_utility)
            == len(ending_preds)
            == len(ending_utility)
        )
        assert len(golds) == len(starting_preds[0]) == len(ending_preds[0])

        final_starting_preds = ensemble_based_on_utility(
            starting_preds, starting_utility, top_k
        )
        final_ending_preds = ensemble_based_on_utility(
            ending_preds, ending_utility, top_k
        )
        ...
        if starting_eval_flag:
            ...
            final_metrics["starting_best_particle_on_validation"] = (
                starting_best_utility_index
            )
            final_metrics["starting_best_single_test_accuracy"] = accuracy_score(
                golds, starting_preds[starting_best_utility_index]
            )
            final_metrics["starting_top-k_ensemble_test_accuracy"] = accuracy_score(
                golds, final_starting_preds
            )
            ...
            print(
                "starting best single test accuracy: ",
                accuracy_score(golds, starting_preds[starting_best_utility_index]),
            )
            print(
                "starting top-k ensemble test accuracy: ",
                accuracy_score(golds, final_starting_preds),
            )
        ...
        final_metrics["ending_best_single_test_accuracy"] = accuracy_score(
            golds, ending_preds[ending_best_utility_index]
        )
        final_metrics["ending_top-k_ensemble_test_accuracy"] = accuracy_score(
            golds, final_ending_preds
        )
        ...
        print(
            "ending best single test accuracy: ",
            accuracy_score(golds, ending_preds[ending_best_utility_index]),
        )
        print(
            "ending top-k ensemble test accuracy: ",
            accuracy_score(golds, final_ending_preds),
        )

    elif (
        eval_type == "exact_match"
        or eval_type == "external_api"
        or eval_type == "perplexity"
        or eval_type == "rm_default"
        or eval_type == "rm_concise"
        or eval_type == "rm_verbose"
        or eval_type == "human"
    ):
        starting_scores = []
        starting_utility = []
        ...
        for i in range(len(particle_paths)):
            if "particle_" + str(i) in particle_paths:
                particle_path = "particle_" + str(i)
                scores_file = search_path / particle_path / "personal_best" / "scores.json"
        ...
        try:
            for i in range(len(particle_paths)):
                if "particle_" + str(i) in particle_paths:
                    particle_path = "particle_" + str(i)
                    scores_file = search_path / particle_path / "now" / "scores.json"
                    with open(scores_file, "r") as f:
                        particle_data = json.load(f)
        ...
        if starting_eval_flag:
            ...
            final_metrics["starting_best_single_test_accuracy"] = sum(
                starting_scores[starting_best_utility_index]
            ) / len(starting_scores[starting_best_utility_index])
            if (
                eval_type == "exact_match"
                or eval_type == "rm_default"
                or eval_type == "rm_concise"
                ...
                temp_scores = ensemble_based_on_utility(
                    starting_scores, starting_utility, top_k
                )
                final_metrics["starting_top-k_ensemble_test_accuracy"] = sum(
                    temp_scores
                ) / len(temp_scores)
                ...
                # average the scores
                final_metrics["starting_top-k_ensemble_test_accuracy"] = avg(
                    [sum(x) / len(x) for x in retained_scores_list]
                )
                final_metrics["starting_top-k_ensemble_test_accuracy"] = avg(
                    [sum(x) / len(x) for x in retained_scores_list]
                )
            ...
            print(
                "starting best validation utility: ",
                final_metrics["starting_best_validation_utility"],
            )
            print("starting best particle on validation: ", starting_best_utility_index)
            print(
                "starting best single test accuracy: ",
                final_metrics["starting_best_single_test_accuracy"],
            )
            print(
                "starting top-k ensemble test accuracy: ",
                final_metrics["starting_top-k_ensemble_test_accuracy"],
            )

        final_metrics["ending_best_validation_utility"] = max(ending_utility)
        final_metrics["ending_best_particle_on_validation"] = ending_best_utility_index
        final_metrics["ending_best_single_test_accuracy"] = sum(
            ending_scores[ending_best_utility_index]
        ) / len(ending_scores[ending_best_utility_index])
        if (
            eval_type == "exact_match"
            or eval_type == "rm_default"
            or eval_type == "rm_concise"
            ...
            temp_scores = ensemble_based_on_utility(
                ending_scores, ending_utility, top_k
            )
            final_metrics["ending_top-k_ensemble_test_accuracy"] = sum(
                temp_scores
            ) / len(temp_scores)
        elif eval_type == "external_api":
            ...
            # average the scores
            final_metrics["ending_top-k_ensemble_test_accuracy"] = avg(
                [sum(x) / len(x) for x in retained_scores_list]
            )
            final_metrics["ending_top-k_ensemble_test_accuracy"] = avg(
                [sum(x) / len(x) for x in retained_scores_list]
        ...
        print(
            "ending best validation utility: ",
            final_metrics["ending_best_validation_utility"],
        )
        print(
            "ending best particle on validation: ",
            final_metrics["ending_best_particle_on_validation"],
        )
        print(
            "ending best single test accuracy: ",
            final_metrics["ending_best_single_test_accuracy"],
        )
        print(
            "ending top-k ensemble test accuracy: ",
            final_metrics["ending_top-k_ensemble_test_accuracy"],
        )

    # eval_type independent part of g_history analysis

    utility_file = Path("search") / name / "utility_scratchpad.json"
    with open(utility_file, "r") as f:
